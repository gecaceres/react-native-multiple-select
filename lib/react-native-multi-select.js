...
export default class MultiSelect extends Component {
  ...
  static defaultProps = {
    ...
    noResultText: 'No items to display',
  ...
  _getSelectLabel = () => {
    const { selectText, single, selectedItems, displayKey, selectTextInfo } = this.props;
  ...
  _addItem = () => {
    ...
    const match = this._filterItems(this.state.searchTerm);
    if (match.length >= 1) {
      this._toggleItem(match[0]);
      /*onSelectedItemsChange([...selectedItems, match[0][uniqueKey]]);*/
      this._clearSearchTerm();
    }
    /*let newItems = [];
  ...
  _filterItems = searchTerm => {
    let renderItems;
    const { filterMethod, removeSelected } = this.props;
    switch (filterMethod) {
      case 'full':
        renderItems = this._filterItemsFull(searchTerm);
        break
      default:
        renderItems = this._filterItemsPartial(searchTerm);
        break
    }
    if (removeSelected) {
  ...
  _renderItems = () => {
    ...
    let itemList;
    let addItemRow;
    let renderItems = searchTerm ? this._filterItems(searchTerm) : [];
    ...
    if (renderItems.length > 0) {
      ...
      searchTermMatch = renderItems.filter(item => item.name === searchTerm)
        .length;
    } else if (!canAddItems && searchTerm) {
      itemList = (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
            style={[
